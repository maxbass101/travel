...
***************************************************************************** */
    ...
    let storiesElement, categoriesElements;
    function windowLoad() {
        storiesElement = document.querySelector(".stories");
        categoriesElements = document.querySelectorAll(".stories__items");
        if (storiesElement) loadArticles();
    }
    async function loadArticles(start = 0, limit = 4) {
        const articlesJson = "files/json/stories.json";
        const response = await fetch(articlesJson, {
            metod: "GET"
        });
        if (response.ok) {
            const results = await response.json();
            setArticles(results, start, limit);
        }
    }
    function setArticles(articles, start, limit) {
        const articlesArray = articles.stories;
        let articleTemplate;
        articlesArray.forEach(((article, index) => {
            if (index >= start && index < limit + start) {
                articleTemplate = `\n         <article class="stories__item item">\n            <a href="${article.url}" target="_blank" class="item__image">\n               <img src="${article.image}" alt="image">\n            </a>\n            <div class="item__title">${article.title}</div>\n            <div class="item__text">\n               ${article.text}\n            </div>\n            <a href="${article.url}" target="_blank" class="item__link">${article.link}</a>\n         </article>\n         `;
                const category = article.category;
                let collectionItems;
                if (1 === category) collectionItems = document.querySelector("#cat-01");
                if (2 === category) collectionItems = document.querySelector("#cat-02");
                if (3 === category) collectionItems = document.querySelector("#cat-03");
                collectionItems.insertAdjacentHTML("beforeend", articleTemplate);
            }
        }));
    }
    let counter = 1;
    document.addEventListener("click", documentActions, counter);
    function documentActions(e) {
        const targetElement = e.target;
        if (targetElement.closest(".stories__button")) {
            const articlesQuantity = document.querySelectorAll(".stories__item").length;
            loadArticles(articlesQuantity);
            if (1 === counter) document.querySelector(".stories__button").style.display = "none"; else counter++;
            e.preventDefault();
        }
    }
    window["FLS"] = true;
